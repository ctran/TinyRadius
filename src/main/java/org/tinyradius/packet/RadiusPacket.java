...
public class RadiusPacket {
	...
	protected static RadiusPacket decodePacket(Dictionary dictionary, InputStream in, String sharedSecret, RadiusPacket request) throws IOException,
		...
		final int type = in.read() & 0x0ff;
		final int identifier = in.read() & 0x0ff;
		final int length = (in.read() & 0x0ff) << 8 | (in.read() & 0x0ff);
		...
		final byte[] attributeData = new byte[length - RADIUS_HEADER_LENGTH];
		...
		while (pos < attributeData.length) {
			...
			final int attributeLength = attributeData[pos + 1] & 0x0ff;
		...
		final RadiusPacket rp = createRadiusPacket(type);
		rp.setPacketType(type);
		...
		pos = 0;
		while (pos < attributeData.length) {
			final int attributeType = attributeData[pos] & 0x0ff;
			final int attributeLength = attributeData[pos + 1] & 0x0ff;
			final RadiusAttribute a = RadiusAttribute.createRadiusAttribute(dictionary, -1, attributeType);
	 ...
	 */
	private List<RadiusAttribute> attributes = new ArrayList<RadiusAttribute>();
	...
	public void addAttribute(RadiusAttribute attribute) {
		...
		if (attribute.getVendorId() == -1) {
			...
			final VendorSpecificAttribute vsa = new VendorSpecificAttribute(attribute.getVendorId());
	...
	public void addAttribute(String typeName, String value) {
		...
		final AttributeType type = dictionary.getAttributeTypeByName(typeName);
		if (type == null) {
			throw new IllegalArgumentException("unknown attribute type '" + typeName + "'");
		}

		final RadiusAttribute attribute = RadiusAttribute.createRadiusAttribute(getDictionary(), type.getVendorId(), type.getTypeCode());
	...
	protected void checkResponseAuthenticator(String sharedSecret, int packetLength, byte[] attributes, byte[] requestAuthenticator)
	        throws RadiusException {
		final byte[] authenticator = createResponseAuthenticator(sharedSecret, packetLength, attributes, requestAuthenticator);
	...
	protected byte[] createRequestAuthenticator(String sharedSecret) {
		final byte[] secretBytes = RadiusUtil.getUtf8Bytes(sharedSecret);
	...
	protected void encodePacket(OutputStream out, String sharedSecret, RadiusPacket request) throws IOException {
		...
		final int packetLength = RADIUS_HEADER_LENGTH + attributes.length;
	...
	public RadiusAttribute getAttribute(int type) {
		final List<RadiusAttribute> attrs = getAttributes(type);
		if (attrs.size() > 1) {
	...
	public RadiusAttribute getAttribute(int vendorId, int type) {
		...
		final List<RadiusAttribute> attrs = getAttributes(vendorId, type);
		if (attrs.size() > 1) {
			throw new RuntimeException("multiple attributes of requested type " + type);
		} else if (attrs.size() == 0) {
			return null;
	...
	public RadiusAttribute getAttribute(String type) {
		...
		final AttributeType t = dictionary.getAttributeTypeByName(type);
		if (t == null) {
	...
	protected byte[] getAttributeBytes() throws IOException {
		final ByteArrayOutputStream bos = new ByteArrayOutputStream(MAX_PACKET_LENGTH);
		for (final RadiusAttribute a : attributes) {
	...
	public List<RadiusAttribute> getAttributes(int attributeType) {
		...
		final LinkedList<RadiusAttribute> result = new LinkedList<RadiusAttribute>();
		for (final RadiusAttribute a : attributes) {
	...
	public List<RadiusAttribute> getAttributes(int vendorId, int attributeType) {
		...
		final LinkedList<RadiusAttribute> result = new LinkedList<RadiusAttribute>();
		final List<VendorSpecificAttribute> vsas = getVendorAttributes(vendorId);
		for (final VendorSpecificAttribute vsa : vsas) {
			final List<RadiusAttribute> sas = vsa.getSubAttributes();
			for (final RadiusAttribute attr : sas) {
	...
	public String getAttributeValue(String type) {
		final RadiusAttribute attr = getAttribute(type);
		if (attr == null) {
			return null;
		}
	...
	public VendorSpecificAttribute getVendorAttribute(int vendorId) {
		for (final Object element : getAttributes(VendorSpecificAttribute.VENDOR_SPECIFIC)) {
			final VendorSpecificAttribute vsa = (VendorSpecificAttribute) element;
	...
	public List<VendorSpecificAttribute> getVendorAttributes(int vendorId) {
		final LinkedList<VendorSpecificAttribute> result = new LinkedList<VendorSpecificAttribute>();
		for (final RadiusAttribute a : attributes) {
	...
	public void removeAttribute(RadiusAttribute attribute) {
		if (attribute.getVendorId() == -1) {
			...
			final List<VendorSpecificAttribute> vsas = getVendorAttributes(attribute.getVendorId());
			for (final VendorSpecificAttribute vsa : vsas) {
				final List<RadiusAttribute> sas = vsa.getSubAttributes();
	...
	public void removeAttributes(int type) {
		...
		final Iterator<RadiusAttribute> i = attributes.iterator();
		while (i.hasNext()) {
			final RadiusAttribute attribute = i.next();
	...
	public void removeAttributes(int vendorId, int typeCode) {
		...
		for (final VendorSpecificAttribute vsa : vsas) {
			final List<RadiusAttribute> sas = vsa.getSubAttributes();
			for (final Iterator<RadiusAttribute> j = sas.iterator(); j.hasNext();) {
				final RadiusAttribute attr = j.next();
	...
	public void removeLastAttribute(int type) {
		...
		final RadiusAttribute lastAttribute = attrs.get(attrs.size() - 1);
	...
	public void setDictionary(Dictionary dictionary) {
		...
		for (final Object element : attributes) {
			final RadiusAttribute attr = (RadiusAttribute) element;
