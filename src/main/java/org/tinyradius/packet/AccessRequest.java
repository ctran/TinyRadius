...
public class AccessRequest extends RadiusPacket {
	...
	private String decodePapPassword(byte[] encryptedPass, byte[] sharedSecret) throws RadiusException {
		...
		final byte[] passtrunc = new byte[len];
	...
	protected void decodeRequestAttributes(String sharedSecret) throws RadiusException {
		...
		final RadiusAttribute chapChallenge = getAttribute(CHAP_CHALLENGE);

		if (userPassword != null) {
			setAuthProtocol(AUTH_PAP);
			password = decodePapPassword(userPassword.getAttributeData(), RadiusUtil.getUtf8Bytes(sharedSecret));
			// copy truncated data
	...
	private byte[] encodeChapPassword(String plaintext, byte[] chapChallenge) {
		...
		final byte chapIdentifier = (byte) random.nextInt(256);
		final byte[] chapPassword = new byte[17];
		...
		final byte[] chapHash = md5.digest(chapChallenge);
	...
	protected void encodeRequestAttributes(String sharedSecret) {
		...
		if (getAuthProtocol().equals(AUTH_PAP)) {
			final byte[] pass = encodePapPassword(RadiusUtil.getUtf8Bytes(password), RadiusUtil.getUtf8Bytes(sharedSecret));
			...
			final byte[] challenge = createChapChallenge();
			final byte[] pass = encodeChapPassword(password, challenge);
	...
	private boolean verifyChapPassword(String plaintext) throws RadiusException {
		...
		final byte chapIdentifier = chapPassword[0];
		final MessageDigest md5 = getMd5Digest();
		md5.reset();
		md5.update(chapIdentifier);
		md5.update(RadiusUtil.getUtf8Bytes(plaintext));
