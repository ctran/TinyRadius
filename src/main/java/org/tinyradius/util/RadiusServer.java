...
public abstract class RadiusServer {
	...
	private final List<ReceivedPacket> receivedPackets = new LinkedList<ReceivedPacket>();
	...
	public RadiusPacket accessRequestReceived(AccessRequest accessRequest, InetSocketAddress client) throws RadiusException {
		final String plaintext = getUserPassword(accessRequest.getUserName());
		...
		final RadiusPacket answer = new RadiusPacket(type, accessRequest.getPacketIdentifier());
	...
	public RadiusPacket accountingRequestReceived(AccountingRequest accountingRequest, InetSocketAddress client) throws RadiusException {
		final RadiusPacket answer = new RadiusPacket(RadiusPacket.ACCOUNTING_RESPONSE, accountingRequest.getPacketIdentifier());
	...
	protected void copyProxyState(RadiusPacket request, RadiusPacket answer) {
		final List<RadiusAttribute> proxyStateAttrs = request.getAttributes(33);
		for (final Object element : proxyStateAttrs) {
			final RadiusAttribute proxyStateAttr = (RadiusAttribute) element;
	...
	protected boolean isPacketDuplicate(RadiusPacket packet, InetSocketAddress address) {
		...
		final long intervalStart = now - getDuplicateInterval();

		final byte[] authenticator = packet.getAuthenticator();

		synchronized (receivedPackets) {
			for (final Iterator<ReceivedPacket> i = receivedPackets.iterator(); i.hasNext();) {
				final ReceivedPacket p = i.next();
			...
			final ReceivedPacket rp = new ReceivedPacket();
			rp.address = address;
	...
	protected DatagramPacket makeDatagramPacket(RadiusPacket packet, String secret, InetAddress address, int port, RadiusPacket request)
		...
		final DatagramPacket datagram = new DatagramPacket(data, data.length, address, port);
		return datagram;
	}
