...
public abstract class RadiusProxy extends RadiusServer {
	 ...
	 */
	private final Map<String, RadiusProxyConnection> proxyConnections = new HashMap<String, RadiusProxyConnection>();
	...
	protected RadiusPacket handlePacket(InetSocketAddress localAddress, InetSocketAddress remoteAddress, RadiusPacket request, String sharedSecret)
		...
		final RadiusEndpoint radiusClient = new RadiusEndpoint(remoteAddress, sharedSecret);
		final RadiusEndpoint radiusServer = getProxyServer(request, radiusClient);
	...
	protected void proxyPacket(RadiusPacket packet, RadiusProxyConnection proxyConnection) throws IOException {
		...
		final byte[] auth = packet.getAuthenticator();
	...
	protected void proxyPacketReceived(RadiusPacket packet, InetSocketAddress remote) throws IOException, RadiusException {
		...
		final List<RadiusAttribute> proxyStates = packet.getAttributes(33);
		...
		final RadiusProxyConnection proxyConnection = proxyConnections.remove(state);
